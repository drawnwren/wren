...
| Cross-cutting gates | A/B1 ≤10 ms; per-real-command input-to-desired-frame p99 <4 ms; task yield/cancel and terminal-write distributions; provider freshness/queue depth; snapshot retention; memory; real-OpenSSH remote convergence/save; crash matrix; fuzz/loom/netem jobs. `--gate` refuses non-bare-metal, unpinned, or (for system gates) non-netem/simulated execution. | `wren-startup`, `wren-latency`, `wren-system-gates`, conformance, fuzz targets, loom test, `scripts/netem-remote.sh` |
