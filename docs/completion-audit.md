...
| Phase 1 — native editor | Runnable TUI; transactional modal engine; registers/marks/macros/repeat; search; published Ex v1; buffers/splits/tabs; latest-wins presenter; safe open/save; background recovery WAL; terminal clipboard routing | `cargo test -p wren-engine -p wren-view -p wren-presenter -p wren-tui`; real-file and PTY tests |
| Phase 2 — replicated session and providers | Checksummed session journal, durable dedup, Received/Durable ordering, leases, replay/snapshot resume, bounded event retention, shared heads, Unix daemon, restartable provider process, language-bundle generations, completion/picker/decorations, trust and derived-state query shapes | `cargo test -p wren-session -p wren-sessiond -p wren-shmem -p wren-provider -p wren-config -p wren-derived` |
...
| Phase 4 — remote | Shell-safe dual OpenSSH control/bulk launch, negotiated stdio agent, journaled mutations and persisted saves, Merkle namespace, BLAKE3/FastCDC cache, cached/progressive open states, dirty overlays, tiered coherence/hash budget, search, and three-way reconciliation | `cargo test -p wren-remote -p wren-sessiond --test remote_stdio`; system remote convergence/save histogram |
...
| Cross-cutting gates | A/B1 ≤10 ms; input-to-desired-frame p99 <4 ms; task yield/cancel and terminal-write distributions; provider freshness/queue depth; snapshot retention; memory; remote convergence/save; crash matrix; fuzz/loom/netem jobs | `wren-startup`, `wren-latency`, `wren-system-gates`, conformance, fuzz targets, loom test, `scripts/netem-remote.sh` |
