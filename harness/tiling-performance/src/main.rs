...
use wren_benchmark_support::{
    ArgumentCursor, CommonArguments, bare_metal_declared, distribution, emit_report, histogram,
    pin_requested_cpu, require_bare_metal_cpu, ten_percent_cut,
};
...
fn isolate_probe(iterations: u64) -> Result<TilingPerformanceReport> {
    ...
    anyhow::ensure!(
        ...
        "tiling performance probe failed: {}",
        String::from_utf8_lossy(&output.stderr)
    );
    let report: TilingPerformanceReport =
        serde_json::from_slice(&output.stdout).context("decode tiling performance probe")?;
    anyhow::ensure!(report.schema == 1, "unsupported tiling probe schema");
    anyhow::ensure!(
        report.requested_iterations == iterations,
        "tiling probe iteration count changed"
    );
    anyhow::ensure!(
        u64::try_from(report.samples.len()).unwrap_or(u64::MAX)
            == iterations.saturating_mul(SCENARIO_GATES.len() as u64),
...
fn metrics(report: &TilingPerformanceReport) -> Result<BTreeMap<Box<str>, ScenarioMetrics>> {
    ...
    for sample in &report.samples {
        ...
        scenario
            .terminal_bytes
            .record(sample.terminal_bytes.max(1))?;
        scenario.terminal_patches.record(
            u64::try_from(sample.terminal_patches)
                .unwrap_or(u64::MAX)
                .max(1),
        )?;
    }
...
fn stage_report(histogram: &Histogram<u64>, baseline: StageBaseline) -> (Value, bool) {
    ...
    let maximum_gate_nanos = baseline
        .maximum_hard_gate
        .then(|| ten_percent_cut(baseline.maximum_nanos));
    ...
    let maximum_passed =
        maximum_gate_nanos.map(|maximum_gate_nanos| observed_maximum_nanos < maximum_gate_nanos);
...
fn scenario_report(metrics: &BTreeMap<Box<str>, ScenarioMetrics>) -> Result<(Value, bool)> {
    anyhow::ensure!(
        metrics.len() == SCENARIO_GATES.len(),
        "tiling probe returned {} scenarios, expected {}",
        metrics.len(),
        SCENARIO_GATES.len()
    );
    ...
    for gate in SCENARIO_GATES {
        let metrics = metrics
            .get(gate.name)
            .with_context(|| format!("tiling probe omitted scenario {}", gate.name))?;
...
fn main() -> Result<()> {
    ...
    if process_arguments.first().map(String::as_str) == Some("--probe-child") {
        let iterations = process_arguments
            .get(1)
            .context("probe child requires an iteration count")?
            .parse::<u64>()
            .context("invalid probe child iteration count")?;
        serde_json::to_writer(
            std::io::stdout().lock(),
            &run_tiling_performance_probe(iterations)?,
        )?;
        return Ok(());
    }
    ...
    anyhow::ensure!(
        metrics
            .values()
            .all(|scenario| scenario.full.len() == arguments.iterations),
    ...
    let presenter_passed = probe.dropped_frames == 0
        ...
        && probe.published_frames
            == probe
                .setup_presentations
                .saturating_add(arguments.iterations.saturating_mul(4));
