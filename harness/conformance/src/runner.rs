...
const SCENARIOS: &[Scenario] = &[
    ...
    Scenario {
        name: "counts-and-text-objects",
        lines: &["alpha beta gamma", "delta epsilon", "zeta"],
        keys: &["2w", "d2w", "u", "ciwX<Esc>"],
    },
    Scenario {
        name: "marks",
        lines: &["alpha", "beta", "gamma"],
    ...
    Scenario {
        name: "macros-and-repeat",
        lines: &["alpha", "beta", "gamma"],
        keys: &["qaA!<Esc>q", "j", "@a", "."],
    },
    Scenario {
        name: "single-key-edits",
        lines: &["alpha", "beta", "gamma"],
        keys: &["x", "u", "rZ", "J", "u", "<C-R>"],
    },
    Scenario {
        name: "visual-operators",
...
pub fn record_goldens(root: &Path) -> Result<PathBuf> {
    ...
    for scenario in SCENARIOS {
        ...
        let trace = GoldenTrace {
            oracle_version: oracle.version().to_owned(),
...
pub fn check_wren_against_goldens() -> Result<()> {
    ...
    for scenario in SCENARIOS {
        ...
        let golden: GoldenTrace = serde_json::from_slice(
...
fn feed_wren(editor: &mut Editor<DefaultText>, input: &str) -> Result<()> {
    ...
    while !rest.is_empty() {
        ...
        editor
            .handle_key(key)
            .with_context(|| format!("wren rejected key trace {input:?}"))?;
        rest = after;
    }
    Ok(())
}

fn compare_core_state(
    scenario: &str,
    step: &str,
    ...
    if pending != expected.pending_operator {
        differences.push(format!(
            "pending operator expected {:?}, got {pending:?}",
            expected.pending_operator
        ));
    }
    if buffer != expected.buffer {
        differences.push(format!(
            "buffer expected {:?}, got {buffer:?}",
            expected.buffer
        ));
    }
    if actual_cursor != expected.cursor {
        differences.push(format!(
            "cursor expected {}, got {actual_cursor}",
            expected.cursor
        ));
    }
    ...
    if differences.is_empty() {
        ...
        bail!(
            "Neovim differential mismatch in {scenario} after {step:?}: {}",
            differences.join("; ")
        )
    }
}

fn compare_changelist(
    editor: &Editor<DefaultText>,
    expected: &Value,
    differences: &mut Vec<String>,
) {
    let Some(parts) = expected.as_array() else {
        differences.push("oracle changelist is not an array".to_owned());
        return;
    };
    let expected_entries = parts
        ...
        .filter_map(|entry| {
            Some((
                usize::try_from(entry.get("lnum")?.as_u64()?)
                    .ok()?
                    .saturating_sub(1),
                usize::try_from(entry.get("col")?.as_u64()?).ok()?,
            ))
        })
        .collect::<Vec<_>>();
    let expected_index = parts
        .get(1)
        .and_then(Value::as_u64)
        .and_then(|value| usize::try_from(value).ok())
        .unwrap_or(0);
    let actual_entries = editor
        .changelist()
        .map(|byte| line_column(editor, byte))
        .collect::<Vec<_>>();
    // Neovim's order flips around the current changelist index after some
    // destructive commands; compare the active edge while retaining topology.
    let same_tail = actual_entries.last() == expected_entries.first()
        || actual_entries.last() == expected_entries.last();
    let same_empty_state = actual_entries.is_empty() == expected_entries.is_empty();
    let same_index_state = (editor.change_index() == 0) == (expected_index == 0);
    if !same_tail || !same_empty_state || !same_index_state {
        differences.push(format!(
            "changelist expected ({expected_entries:?}, {expected_index}), got ({actual_entries:?}, {})",
            editor.change_index()
        ));
    }
}
...
fn compare_options(expected: &Value, differences: &mut Vec<String>) {
    ...
    if &actual != expected {
        differences.push(format!(
            "semantics-affecting options expected {expected}, got {actual}"
        ));
    }
}
...
fn compare_selections(
    ...
    let actual = editor.last_visual_selection().map(|selection| {
        (
    ...
    if expected.is_some() && actual != expected {
        differences.push(format!(
            "last visual selection expected {expected:?}, got {actual:?}"
        ));
    }
}
...
fn compare_marks(editor: &Editor<DefaultText>, expected: &Value, differences: &mut Vec<String>) {
    ...
    if actual != expected_marks {
        differences.push(format!(
            "named marks expected {expected_marks:?}, got {actual:?}"
        ));
    }
}
...
fn compare_jumplist(editor: &Editor<DefaultText>, expected: &Value, differences: &mut Vec<String>) {
    let Some(parts) = expected.as_array() else {
        differences.push("oracle jumplist is not an array".to_owned());
        return;
    };
    let expected_entries = parts
        .first()
    ...
    let actual_entries = editor
        ...
        .map(|byte| line_column(editor, byte))
        .collect::<Vec<_>>();
    let same_tail = actual_entries.last() == expected_entries.last();
    let same_empty_state = actual_entries.is_empty() == expected_entries.is_empty();
    let same_index_state = (editor.jump_index() == 0) == (expected_index == 0);
    if !same_tail || !same_empty_state || !same_index_state {
        differences.push(format!(
            "jumplist expected ({expected_entries:?}, {expected_index}), got ({actual_entries:?}, {})",
            editor.jump_index()
        ));
    }
}
...
fn compare_search(editor: &Editor<DefaultText>, expected: &Value, differences: &mut Vec<String>) {
    ...
    let actual = editor
        .last_search()
        .map_or(("", true), |(pattern, direction)| {
            (pattern, direction == SearchDirection::Forward)
        });
    if actual != (pattern, forward) {
        differences.push(format!(
            "search expected ({pattern:?}, {forward}), got {actual:?}"
        ));
    }
}
...
fn compare_messages(editor: &Editor<DefaultText>, expected: &str, differences: &mut Vec<String>) {
    ...
    if actual_nonempty == expected.is_empty() {
        differences.push(format!(
            "message-log emptiness expected {}, got {actual_nonempty}",
            !expected.is_empty()
        ));
    }
}

fn compare_undo_tree(
    editor: &Editor<DefaultText>,
    expected: &Value,
    differences: &mut Vec<String>,
) {
    let expected_total = expected
        .get("entries")
        .and_then(Value::as_array)
        .map_or(0, |entries| count_undo_entries(entries));
    let expected_at_head = expected_total == 0
        || expected.get("seq_cur").and_then(Value::as_u64)
            == expected.get("seq_last").and_then(Value::as_u64);
    ...
    entries
        ...
        .map(|entry| {
            1 + entry
                .get("alt")
                .and_then(Value::as_array)
                .map_or(0, |alternate| count_undo_entries(alternate))
        })
        .sum()
}
...
fn compare_registers(
    ...
    for expected in registers {
        ...
        if actual_text != expected_text
            || (!expected_type.is_empty() && actual_type != expected_type)
        {
            differences.push(format!(
                "register {name:?} expected ({expected_type:?}, {expected_text:?}), got ({actual_type:?}, {actual_text:?})"
            ));
        }
    }
}
