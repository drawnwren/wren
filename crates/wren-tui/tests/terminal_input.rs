...
fn editor_session(contents: &str) -> (TempDir, NamedTempFile, PtySession) {
    ...
    file.write_all(contents.as_bytes()).expect("write editor document");
...
fn wait_for_screen(session: &mut PtySession, description: &str, predicate: impl Fn(&str) -> bool) {
    ...
    loop {
        ...
        assert!(Instant::now() < deadline, "timed out waiting for {description}; screen was:\n{screen}");
...
fn workspace_lsp_starts_on_open_even_while_input_remains_busy() {
    ...
    std::fs::write(&server, include_str!("fixtures/fake_rust_analyzer.py")).expect("write fake rust-analyzer");
    let mut permissions = std::fs::metadata(&server).expect("fake server metadata").permissions();
    ...
    let path_variable = format!("PATH={}:{}", bin.display(), std::env::var("PATH").unwrap_or_default());
    ...
    let startup_log = loop {
        ...
        assert!(Instant::now() < deadline, "workspace LSP did not start while input remained active; log: {current}");
        thread::sleep(Duration::from_millis(10));
    };

    assert!(startup_log.contains("initialize file://"));
    let workspace = std::fs::canonicalize(std::env::current_dir().expect("current workspace")).expect("canonical workspace");
    assert!(startup_log.contains(workspace.to_string_lossy().as_ref()), "LSP was not rooted at the launch workspace: {startup_log}");
    assert!(!startup_log.contains("textDocument/hover"));
    wait_for_screen(&mut session, "editor readiness after workspace LSP startup", |screen| screen.contains("NORMAL") && screen.contains("fn main"));
...
fn real_terminal_keyboard_and_mouse_enter_visual_mode() {
    ...
    wait_for_screen(&mut session, "initial Normal mode", |screen| screen.contains("NORMAL") && screen.contains("abcdef"));

    session.send_input(b"vl").expect("enter keyboard Visual mode");
    wait_for_screen(&mut session, "keyboard Visual mode", |screen| screen.contains("VISUAL"));
    session.send_input(b"d").expect("delete the keyboard selection");
    wait_for_screen(&mut session, "keyboard Visual selection deletion", |screen| {
        screen.contains("NORMAL") && screen.contains("cdef") && !screen.contains("abcdef")
    });
    session.send_input(b"u").expect("restore keyboard deletion");
    wait_for_screen(&mut session, "restored document", |screen| screen.contains("abcdef"));
    ...
    session.send_input(b"\x1b[<0;5;1M\x1b[<32;8;1M\x1b[<0;8;1m").expect("drag across editor cells");
    wait_for_screen(&mut session, "mouse Visual mode", |screen| screen.contains("VISUAL"));
    session.send_input(b"d").expect("delete the mouse selection");
    wait_for_screen(&mut session, "mouse Visual selection deletion", |screen| screen.contains("NORMAL") && screen.contains("af") && !screen.contains("abcdef"));
...
fn real_terminal_unnamedplus_paste_reads_osc52() {
    ...
    wait_for_screen(&mut session, "initial document", |screen| screen.contains("NORMAL") && screen.contains("alpha"));
    ...
    session.send_input(b"\x1b]52;c;Y2xpcA==\x1b\\").expect("reply to clipboard query");
    wait_for_screen(&mut session, "OSC 52 clipboard paste", |screen| screen.contains("acliplpha"));
...
fn real_terminal_large_rust_file_opens_navigates_and_edits_responsively() {
    ...
    wait_for_screen(&mut session, "large Rust first frame", |screen| screen.contains("NORMAL") && screen.contains("item_00000"));
    let open_elapsed = open_at.elapsed();
    assert!(open_elapsed < Duration::from_secs(2), "large Rust executable open took {open_elapsed:?}");

    let navigation_at = Instant::now();
    session.send_input(b"G").expect("navigate to file end");
    wait_for_screen(&mut session, "large Rust final line", |screen| screen.contains("item_13999"));
    let navigation_elapsed = navigation_at.elapsed();
    assert!(navigation_elapsed < Duration::from_millis(250), "large Rust executable navigation took {navigation_elapsed:?}");

    let edit_at = Instant::now();
    session.send_input(b"iX\x1b").expect("edit the large Rust final line");
    wait_for_screen(&mut session, "large Rust edited final line", |screen| screen.contains("[+]") && screen.lines().any(|line| line.contains("140X")));
    let edit_elapsed = edit_at.elapsed();
    assert!(edit_elapsed < Duration::from_millis(250), "large Rust executable edit took {edit_elapsed:?}");
