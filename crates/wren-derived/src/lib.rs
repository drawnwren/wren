...
use wren_types::{ConfigGeneration, DocumentId, DocumentRevision, ProviderGeneration, WorkspaceGeneration, WorkspaceGenerationKind};
...
impl DerivedStateDb {
    ...
    pub fn set_workspace_generation(&mut self, kind: WorkspaceGenerationKind, generation: WorkspaceGeneration) {
        self.workspace_generations.insert(kind, generation);
    }

    pub fn set_provider_generation(&mut self, provider: impl Into<Box<str>>, generation: ProviderGeneration) {
        self.provider_generations.insert(provider.into(), generation);
    }
    ...
    pub fn resolved_language(&self, document_id: DocumentId) -> Option<&str> {
        self.documents.get(&document_id).map(|document| document.language_id.as_ref())
    }

    #[must_use]
    pub fn command_enabled(&self, command: &str, trusted: bool) -> bool {
        let configured = self.config_generation.is_some();
        if command.starts_with("task.") || command.starts_with("extension.") { configured && trusted } else { configured }
    }
    ...
    pub fn outline(&mut self, document_id: DocumentId) -> Option<QueryResult<Vec<OutlineItem>>> {
        ...
        let generation = self.provider_generations.get("syntax").copied().unwrap_or(ProviderGeneration::new(0));
        let key = OutlineKey { document_id, revision: document.revision, provider_generation: generation };
        if let Some(cached) = self.outline_cache.get(&key) {
            return Some(cached.clone());
        }
        let value = Arc::new(compute_outline(&document.text));
        let result = QueryResult { value, computed_at: self.tick() };
        self.outline_cache.insert(key, result.clone());
        Some(result)
    }

    pub fn workspace_symbols(&mut self) -> QueryResult<Vec<WorkspaceSymbol>> {
        let generation = self.workspace_generations.get(&WorkspaceGenerationKind::ProjectIndex).copied().unwrap_or(WorkspaceGeneration::new(0));
        let provider_generation = self.provider_generations.get("project-index").copied().unwrap_or(ProviderGeneration::new(0));
        let key = SymbolsKey { generation, provider_generation };
        if let Some(cached) = self.symbol_cache.get(&key) {
            return cached.clone();
        }
        let mut value = Vec::new();
        for (document_id, document) in &self.documents {
            value.extend(compute_outline(&document.text).into_iter().map(|item| WorkspaceSymbol {
                name: item.name,
                document_id: *document_id,
                byte: item.byte,
            }));
        }
        value.sort_by(|left, right| left.name.cmp(&right.name).then_with(|| left.document_id.cmp(&right.document_id)));
        let result = QueryResult { value: Arc::new(value), computed_at: self.tick() };
    ...
    pub fn git_baseline_generation(&self) -> Option<WorkspaceGeneration> {
        self.workspace_generations.get(&WorkspaceGenerationKind::Git).copied()
    }
    ...
    for line in text.split_inclusive('\n') {
        ...
        for prefix in ["fn ", "struct ", "enum ", "trait ", "class "] {
            if let Some(rest) = trimmed.strip_prefix(prefix) {
                let name = rest.split(|character: char| !character.is_alphanumeric() && character != '_').next().unwrap_or_default();
                if !name.is_empty() {
                    items.push(OutlineItem { name: name.into(), byte: byte + indentation + prefix.len() });
                }
                break;
            }
        }
...
mod tests {
    ...
    fn editing_document_b_does_not_invalidate_document_a_queries() {
        ...
        db.set_document(DocumentId::new(1), DocumentInput { revision: DocumentRevision::new(1), language_id: "rust".into(), text: Arc::from("fn alpha() {}") });
        db.set_document(DocumentId::new(2), DocumentInput { revision: DocumentRevision::new(1), language_id: "rust".into(), text: Arc::from("fn beta() {}") });
        let alpha = db.outline(DocumentId::new(1)).expect("alpha");
        db.set_document(
            DocumentId::new(2),
            DocumentInput { revision: DocumentRevision::new(2), language_id: "rust".into(), text: Arc::from("fn beta_changed() {}") },
        );
    ...
    fn provider_and_domain_generations_invalidate_only_their_query_shapes() {
        ...
        assert_eq!(outline.computed_at, db.outline(DocumentId::new(1)).expect("outline retained").computed_at);
        db.set_provider_generation("syntax", ProviderGeneration::new(2));
        assert_ne!(outline.computed_at, db.outline(DocumentId::new(1)).expect("outline refreshed").computed_at);
    }
