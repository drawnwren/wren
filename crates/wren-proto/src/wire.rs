...
include!(concat!(env!("OUT_DIR"), "/wren.protocol.v1.rs"));
impl Envelope {
    #[must_use]
...
pub fn encode_frame(envelope: &Envelope, limit: usize) -> Result<Vec<u8>, ProtocolError> {
    ...
    if encoded_len > limit {
        return Err(ProtocolError::FrameTooLarge { actual: encoded_len, limit });
    }
    let mut bytes = Vec::with_capacity(encoded_len + 10);
    envelope.encode_length_delimited(&mut bytes).map_err(|error| ProtocolError::Decode(error.to_string()))?;
    Ok(bytes)
}

pub fn decode_frame(mut bytes: &[u8], limit: usize) -> Result<Envelope, ProtocolError> {
    if bytes.len() > limit.saturating_add(10) {
        return Err(ProtocolError::FrameTooLarge { actual: bytes.len(), limit });
    }
    let envelope = Envelope::decode_length_delimited(&mut bytes).map_err(|error| ProtocolError::Decode(error.to_string()))?;
    if !bytes.is_empty() {
...
pub fn read_envelope(reader: &mut impl Read, limit: usize) -> Result<Option<Envelope>, TransportError> {
    ...
    while byte & 0x80 != 0 {
        ...
        byte = next[0];
        let component = u64::from(byte & 0x7f).checked_shl(shift).ok_or(TransportError::InvalidLengthDelimiter)?;
        length = length.checked_add(component).ok_or(TransportError::InvalidLengthDelimiter)?;
        shift += 7;
        delimiter_bytes += 1;
    }
    let length = usize::try_from(length).map_err(|_| ProtocolError::IntegerOverflow { field: "frame length delimiter" })?;
    if length > limit {
        return Err(ProtocolError::FrameTooLarge { actual: length, limit }.into());
    }
    let mut payload = vec![0_u8; length];
    reader.read_exact(&mut payload)?;
    let envelope = Envelope::decode(payload.as_slice()).map_err(|error| ProtocolError::Decode(error.to_string()))?;
    envelope.validate()?;
    Ok(Some(envelope))
}
...
pub fn write_envelope(writer: &mut impl Write, envelope: &Envelope, limit: usize) -> Result<(), TransportError> {
    ...
    envelope.encode(&mut payload).map_err(|error| ProtocolError::Decode(error.to_string()))?;
...
fn one_char(value: String, field: &'static str) -> Result<char, ProtocolError> {
    ...
    let character = chars.next().ok_or(ProtocolError::InvalidCharacter { field })?;
...
fn hash32(value: Vec<u8>, field: &'static str) -> Result<[u8; 32], ProtocolError> {
    ...
    value.try_into().map_err(|_| ProtocolError::HashLength { field, actual })
}
    ...
    fn from(value: &semantic::Transaction) -> Self {
        Self {
            ...
            edits: value
                .edits()
                .iter()
                .map(|edit| Edit { start: edit.range.start as u64, end: edit.range.end as u64, insert: edit.insert.to_string() })
                .collect(),
        }
    }
}
...
impl TryFrom<Transaction> for semantic::Transaction {
    ...
    fn try_from(value: Transaction) -> Result<Self, Self::Error> {
        let edits = value
            .edits
            .into_iter()
            .map(|edit| Ok(semantic::Edit::new(checked_usize(edit.start, "Edit.start")?..checked_usize(edit.end, "Edit.end")?, edit.insert)))
...
impl From<&semantic::StateDelta> for StateDelta {
    fn from(value: &semantic::StateDelta) -> Self {
        let value = match value {
            semantic::StateDelta::Register { name, text, linewise } => {
                state_delta::Value::Register(RegisterDelta { name: name.to_string(), text: text.to_string(), linewise: *linewise })
            }
            semantic::StateDelta::SearchPattern(pattern) => state_delta::Value::SearchPattern(pattern.to_string()),
            semantic::StateDelta::SearchDirection { backward } => state_delta::Value::SearchBackward(*backward),
            semantic::StateDelta::CommandHistory(command) => state_delta::Value::CommandHistory(command.to_string()),
            semantic::StateDelta::GlobalMark { name, document_id, anchor } => state_delta::Value::GlobalMark(GlobalMarkDelta {
                name: name.to_string(),
                document_id: document_id.get(),
                byte: anchor.byte as u64,
                right_bias: anchor.bias == semantic::Bias::Right,
            }),
            semantic::StateDelta::UndoBranchHead { document_id, semantic_group_id } => state_delta::Value::UndoBranch(UndoBranchDelta {
                document_id: document_id.get(),
            ...
            semantic::StateDelta::MacroRecording { name, raw_keys, lowered_ir } => {
                state_delta::Value::MacroRecording(MacroRecordingDelta { name: name.to_string(), raw_keys: raw_keys.clone(), lowered_ir: lowered_ir.clone() })
            }
            semantic::StateDelta::JumpList { entries, current } => state_delta::Value::JumpList(JumpListDelta {
                entries: entries
                    .iter()
...
impl TryFrom<StateDelta> for semantic::StateDelta {
    ...
    fn try_from(value: StateDelta) -> Result<Self, Self::Error> {
        match required(value.value, "StateDelta.value")? {
            state_delta::Value::Register(register) => {
                Ok(Self::Register { name: one_char(register.name, "RegisterDelta.name")?, text: register.text.into_boxed_str(), linewise: register.linewise })
            }
            state_delta::Value::SearchPattern(pattern) => Ok(Self::SearchPattern(pattern.into_boxed_str())),
            state_delta::Value::SearchBackward(backward) => Ok(Self::SearchDirection { backward }),
            state_delta::Value::CommandHistory(command) => Ok(Self::CommandHistory(command.into_boxed_str())),
            state_delta::Value::GlobalMark(mark) => Ok(Self::GlobalMark {
                ...
                anchor: semantic::Anchor {
                    ...
                    bias: if mark.right_bias { semantic::Bias::Right } else { semantic::Bias::Left },
                },
            }),
                    ...
                    .map(|entry| {
                        Ok(semantic::DurableJumpEntry {
                            ...
                            anchor: semantic::Anchor {
                                ...
                                bias: if entry.right_bias { semantic::Bias::Right } else { semantic::Bias::Left },
                            },
                            path_hint: entry.path_hint.map(String::into_boxed_str),
                        })
                    })
                    .collect::<Result<Vec<_>, ProtocolError>>()?,
                current: jumps.current.map(|index| checked_usize(index, "JumpListDelta.current")).transpose()?,
            }),
        }
    }
}
...
impl From<&semantic::DocumentMutation> for DocumentMutation {
    fn from(value: &semantic::DocumentMutation) -> Self {
        let (kind, related_group_id) = match value.semantic_group_kind {
            ...
            semantic::SemanticGroupKind::MacroInvocation => (SemanticGroupKind::MacroInvocation, None),
            semantic::SemanticGroupKind::Formatter => (SemanticGroupKind::Formatter, None),
            semantic::SemanticGroupKind::WorkspaceRefactor => (SemanticGroupKind::WorkspaceRefactor, None),
            semantic::SemanticGroupKind::UndoOf(group) => (SemanticGroupKind::UndoOf, Some(group.get())),
            semantic::SemanticGroupKind::RedoOf(group) => (SemanticGroupKind::RedoOf, Some(group.get())),
        };
        Self {
...
impl TryFrom<DocumentMutation> for semantic::DocumentMutation {
    ...
    fn try_from(value: DocumentMutation) -> Result<Self, Self::Error> {
        let tag = SemanticGroupKind::try_from(value.semantic_group_kind)
            .map_err(|_| ProtocolError::InvalidEnum { field: "DocumentMutation.semantic_group_kind", value: value.semantic_group_kind })?;
        ...
        let semantic_group_kind = match tag {
            ...
            SemanticGroupKind::UndoOf => semantic::SemanticGroupKind::UndoOf(required(related_group, "DocumentMutation.related_group_id for UndoOf")?),
            SemanticGroupKind::RedoOf => semantic::SemanticGroupKind::RedoOf(required(related_group, "DocumentMutation.related_group_id for RedoOf")?),
        };
        let mutation = Self {
            ...
            transactions: value.transactions.into_iter().map(semantic::Transaction::try_from).collect::<Result<Vec<_>, _>>()?,
        };
        mutation.validate().map_err(|error| ProtocolError::InvalidTransaction(error.to_string()))?;
        Ok(mutation)
    }
}
...
impl TryFrom<ClientMutation> for semantic::ClientMutation {
    ...
    fn try_from(value: ClientMutation) -> Result<Self, Self::Error> {
        let mutation = Self {
            ...
            state_deltas: value.state_deltas.into_iter().map(semantic::StateDelta::try_from).collect::<Result<Vec<_>, _>>()?,
            documents: value.documents.into_iter().map(semantic::DocumentMutation::try_from).collect::<Result<Vec<_>, _>>()?,
        };
        mutation.validate().map_err(|error| ProtocolError::InvalidTransaction(error.to_string()))?;
        Ok(mutation)
    }
...
impl From<&semantic::MutationResult> for MutationResult {
    fn from(value: &semantic::MutationResult) -> Self {
        let value = match value {
            semantic::MutationResult::Received { mutation_id } => mutation_result::Value::Received(Received { mutation_id: mutation_id.get() }),
            semantic::MutationResult::Durable { mutation_id, client_sequence, session_sequence, documents } => mutation_result::Value::Durable(Durable {
            ...
            semantic::MutationResult::RebaseRequired { mutation_id, document_id, authoritative_revision, delta_since_base } => {
            ...
            semantic::MutationResult::LeaseLost { document_id, current_lease_epoch } => {
                mutation_result::Value::LeaseLost(LeaseLost { document_id: document_id.get(), current_lease_epoch: current_lease_epoch.get() })
            }
            semantic::MutationResult::Conflict { document_id, reason } => {
                mutation_result::Value::Conflict(Conflict { document_id: document_id.get(), reason: reason.to_string() })
            }
        };
        Self { value: Some(value) }
    }
...
impl TryFrom<MutationResult> for semantic::MutationResult {
    ...
    fn try_from(value: MutationResult) -> Result<Self, Self::Error> {
        match required(value.value, "MutationResult.value")? {
            mutation_result::Value::Received(received) => Ok(Self::Received { mutation_id: semantic::MutationId::new(received.mutation_id) }),
                ...
                documents: durable
                    ...
                    .map(|document| {
                        Ok(semantic::AcceptedDocument {
                            ...
                            accepted_revision: semantic::DocumentRevision::new(document.accepted_revision),
                            canonical_transaction_hash: hash32(document.canonical_transaction_hash, "AcceptedDocument.canonical_transaction_hash")?,
                        })
                    })
                    .collect::<Result<Vec<_>, ProtocolError>>()?,
            }),
            mutation_result::Value::RebaseRequired(rebase) => Ok(Self::RebaseRequired {
                ...
                authoritative_revision: semantic::DocumentRevision::new(rebase.authoritative_revision),
                delta_since_base: rebase.delta_since_base.into_iter().map(semantic::Transaction::try_from).collect::<Result<Vec<_>, _>>()?,
            }),
            ...
            mutation_result::Value::Conflict(conflict) => {
                Ok(Self::Conflict { document_id: semantic::DocumentId::new(conflict.document_id), reason: conflict.reason.into_boxed_str() })
            }
        }
    }
}
...
impl TryFrom<LeaseGrant> for semantic::LeaseGrant {
    ...
    fn try_from(value: LeaseGrant) -> Result<Self, Self::Error> {
        let policy = OfflinePolicy::try_from(value.offline_policy)
            .map_err(|_| ProtocolError::InvalidEnum { field: "LeaseGrant.offline_policy", value: value.offline_policy })?;
        Ok(Self {
            document_id: semantic::DocumentId::new(value.document_id),
            lease_epoch: semantic::LeaseEpoch::new(value.lease_epoch),
...
impl From<&semantic::SessionEvent> for SessionEvent {
    fn from(value: &semantic::SessionEvent) -> Self {
        let origin = match value.origin {
            semantic::EventOrigin::Client(client_id) => EventOrigin { kind: EventOriginKind::Client as i32, client_id: Some(client_id.get()) },
            semantic::EventOrigin::Workspace => EventOrigin { kind: EventOriginKind::Workspace as i32, client_id: None },
            semantic::EventOrigin::Recovery => EventOrigin { kind: EventOriginKind::Recovery as i32, client_id: None },
        };
        let payload = match &value.payload {
            semantic::SessionEventPayload::DocumentDelta { document_id, accepted_revision, transactions } => {
            ...
            semantic::SessionEventPayload::StateDelta(delta) => session_event_payload::Value::StateDelta(StateDelta::from(delta)),
            semantic::SessionEventPayload::LeaseChange(grant) => session_event_payload::Value::LeaseChange(LeaseGrant::from(grant)),
            semantic::SessionEventPayload::ExternalChange { document_id, content_hash } => {
                session_event_payload::Value::ExternalChange(ExternalChange { document_id: document_id.get(), content_hash: content_hash.to_vec() })
            }
        };
        Self { session_sequence: value.session_sequence.get(), origin: Some(origin), payload: Some(SessionEventPayload { value: Some(payload) }) }
    }
}
...
impl TryFrom<SessionEvent> for semantic::SessionEvent {
    ...
    fn try_from(value: SessionEvent) -> Result<Self, Self::Error> {
        ...
        let origin_kind = EventOriginKind::try_from(origin.kind).map_err(|_| ProtocolError::InvalidEnum { field: "EventOrigin.kind", value: origin.kind })?;
        let origin = match origin_kind {
            EventOriginKind::Client => semantic::EventOrigin::Client(semantic::ClientId::new(required(origin.client_id, "EventOrigin.client_id")?)),
        ...
        let payload = match required(payload.value, "SessionEventPayload.value")? {
            session_event_payload::Value::DocumentDelta(delta) => semantic::SessionEventPayload::DocumentDelta {
                ...
                transactions: delta.transactions.into_iter().map(semantic::Transaction::try_from).collect::<Result<Vec<_>, _>>()?,
            },
            session_event_payload::Value::StateDelta(delta) => semantic::SessionEventPayload::StateDelta(delta.try_into()?),
            session_event_payload::Value::LeaseChange(grant) => semantic::SessionEventPayload::LeaseChange(grant.try_into()?),
            session_event_payload::Value::ExternalChange(change) => semantic::SessionEventPayload::ExternalChange {
        ...
        Ok(Self { session_sequence: semantic::SessionSequence::new(value.session_sequence), origin, payload })
    }
}

impl From<&semantic::DocumentFrontier> for DocumentFrontier {
    fn from(value: &semantic::DocumentFrontier) -> Self {
        Self { document_id: value.document_id.get(), revision: value.revision.get() }
    }
}

impl From<DocumentFrontier> for semantic::DocumentFrontier {
    fn from(value: DocumentFrontier) -> Self {
        Self { document_id: semantic::DocumentId::new(value.document_id), revision: semantic::DocumentRevision::new(value.revision) }
    }
}
...
impl From<&semantic::Resume> for Resume {
    fn from(value: &semantic::Resume) -> Self {
        Self {
            ...
            document_frontiers: value.document_frontiers.iter().map(DocumentFrontier::from).collect(),
            outstanding_mutation_ids: value.outstanding_mutation_ids.iter().map(|id| id.get()).collect(),
        }
    }
}
...
impl From<Resume> for semantic::Resume {
    fn from(value: Resume) -> Self {
        Self {
            ...
            document_frontiers: value.document_frontiers.into_iter().map(semantic::DocumentFrontier::from).collect(),
            outstanding_mutation_ids: value.outstanding_mutation_ids.into_iter().map(semantic::MutationId::new).collect(),
        }
    }
}
...
impl From<&semantic::ResumeResult> for ResumeResult {
    fn from(value: &semantic::ResumeResult) -> Self {
        let value = match value {
            semantic::ResumeResult::Replay { events } => resume_result::Value::Replay(Replay { events: events.iter().map(SessionEvent::from).collect() }),
            semantic::ResumeResult::SnapshotRequired { new_session_epoch, workspace_generation, document_heads } => {
...
impl TryFrom<ResumeResult> for semantic::ResumeResult {
    ...
    fn try_from(value: ResumeResult) -> Result<Self, Self::Error> {
        match required(value.value, "ResumeResult.value")? {
            resume_result::Value::Replay(replay) => {
                Ok(Self::Replay { events: replay.events.into_iter().map(semantic::SessionEvent::try_from).collect::<Result<Vec<_>, _>>()? })
            }
            resume_result::Value::SnapshotRequired(snapshot) => Ok(Self::SnapshotRequired {
                ...
                workspace_generation: semantic::WorkspaceGeneration::new(snapshot.workspace_generation),
                document_heads: snapshot.document_heads.into_iter().map(semantic::DocumentFrontier::from).collect(),
            }),
        }
    }
}
...
impl From<&semantic::FileIdentity> for FileIdentity {
    fn from(value: &semantic::FileIdentity) -> Self {
        Self { device: value.device, file: value.file, generation: value.generation }
    }
}
...
impl TryFrom<SaveRequest> for semantic::SaveRequest {
    ...
    fn try_from(value: SaveRequest) -> Result<Self, Self::Error> {
        Ok(Self {
            ...
            expected_file_identity: required(value.expected_file_identity, "SaveRequest.expected_file_identity")?.into(),
            expected_content_hash: hash32(value.expected_content_hash, "SaveRequest.expected_content_hash")?,
        })
    }
}
...
mod tests {
    ...
    fn semantic_mutation() -> semantic::ClientMutation {
        semantic::ClientMutation {
            ...
            state_deltas: vec![
                semantic::StateDelta::Register { name: 'a', text: "copied".into(), linewise: false },
                semantic::StateDelta::SearchDirection { backward: true },
                semantic::StateDelta::MacroRecording { name: 'q', raw_keys: vec![1, 2], lowered_ir: vec![3, 4] },
                semantic::StateDelta::JumpList {
                    entries: vec![semantic::DurableJumpEntry {
                        document_id: semantic::DocumentId::new(2),
                        anchor: semantic::Anchor { byte: 7, bias: semantic::Bias::Right },
            ...
            documents: vec![semantic::DocumentMutation {
                ...
                semantic_group_kind: semantic::SemanticGroupKind::UndoOf(semantic::SemanticGroupId::new(6)),
                undo_parent: Some(semantic::SemanticGroupId::new(5)),
                transactions: vec![semantic::Transaction::new(semantic::DocumentRevision::new(5), vec![semantic::Edit::new(1..3, "x")]).expect("transaction")],
            }],
        }
    }
    ...
    fn mutation_round_trips_through_typed_protobuf_frame() {
        ...
        let envelope = Envelope::new(42, envelope::Payload::ClientMutation(ClientMutation::from(&expected)));
    ...
    fn result_event_resume_and_save_contracts_round_trip() {
        ...
        assert_eq!(semantic::MutationResult::try_from(MutationResult::from(&result)).expect("result"), result);

        let event = semantic::SessionEvent {
            session_sequence: semantic::SessionSequence::new(8),
            origin: semantic::EventOrigin::Workspace,
            payload: semantic::SessionEventPayload::ExternalChange { document_id: semantic::DocumentId::new(4), content_hash: [9; 32] },
        };
        assert_eq!(semantic::SessionEvent::try_from(SessionEvent::from(&event)).expect("event"), event);
        ...
        let resume = semantic::ResumeResult::SnapshotRequired {
            ...
            document_heads: vec![semantic::DocumentFrontier { document_id: semantic::DocumentId::new(4), revision: semantic::DocumentRevision::new(5) }],
        };
        assert_eq!(semantic::ResumeResult::try_from(ResumeResult::from(&resume)).expect("resume"), resume);

        let saved = semantic::Saved {
            document_id: semantic::DocumentId::new(4),
            persisted_frontier: semantic::DocumentRevision::new(5),
            new_file_identity: semantic::FileIdentity { device: 1, file: 2, generation: 3 },
            new_content_hash: [7; 32],
        };
        assert_eq!(semantic::Saved::try_from(Saved::from(&saved)).expect("saved"), saved);
    }

    #[test]
    fn rejects_wrong_major_missing_payload_oversize_and_bad_hashes() {
        let mut envelope = Envelope::new(1, envelope::Payload::Hello(Hello { major: 1, minor: 0, capabilities: Vec::new(), max_frame_bytes: 1024 }));
        envelope.protocol_version = 2 << 16;
        assert!(matches!(envelope.validate(), Err(ProtocolError::UnsupportedVersion { .. })));
        envelope.protocol_version = PROTOCOL_VERSION;
        envelope.payload = None;
        assert!(matches!(envelope.validate(), Err(ProtocolError::MissingField(_))));
        let envelope = Envelope::new(1, envelope::Payload::Hello(Hello { major: 1, minor: 0, capabilities: vec!["x".repeat(100)], max_frame_bytes: 1024 }));
        assert!(matches!(encode_frame(&envelope, 8), Err(ProtocolError::FrameTooLarge { .. })));
        let invalid = MutationResult {
            value: Some(mutation_result::Value::Durable(Durable {
                mutation_id: 1,
                client_sequence: 1,
                session_sequence: 1,
                documents: vec![AcceptedDocument { document_id: 1, accepted_revision: 1, canonical_transaction_hash: vec![0; 31] }],
            })),
        };
        assert!(matches!(semantic::MutationResult::try_from(invalid), Err(ProtocolError::HashLength { .. })));
    }
    ...
    fn stream_framing_handles_multiple_messages_clean_eof_and_limits() {
        ...
        let second = Envelope::new(2, envelope::Payload::ClientMutation(ClientMutation::from(&semantic_mutation())));
        ...
        assert_eq!(read_envelope(&mut cursor, DEFAULT_MAX_FRAME_BYTES).expect("read first").expect("first"), first);
        assert_eq!(read_envelope(&mut cursor, DEFAULT_MAX_FRAME_BYTES).expect("read second").expect("second"), second);
        ...
        assert!(matches!(read_envelope(&mut oversized, 8), Err(TransportError::Protocol(ProtocolError::FrameTooLarge { .. }))));
    }
}
