...
pub enum WorkflowError {
    ...
    StaleRevision { expected: DocumentRevision, actual: DocumentRevision },
...
fn task_command(spec: &TaskSpec) -> Command {
...
impl TaskSupervisor {
    ...
    pub fn run_until_cancelled(&self, spec: &TaskSpec, is_cancelled: impl Fn() -> bool) -> Result<TaskOutput, WorkflowError> {
        self.run_captured(spec, None, is_cancelled)
    }

    fn run_captured(&self, spec: &TaskSpec, input: Option<&[u8]>, is_cancelled: impl Fn() -> bool) -> Result<TaskOutput, WorkflowError> {
        if !self.trusted {
        ...
        let input_writer = input
            .map(|input| {
                let mut stdin = child.stdin.take().ok_or_else(|| io::Error::other("task stdin pipe missing"))?;
                let input = input.to_vec();
                Ok::<_, io::Error>(thread::spawn(move || -> io::Result<()> {
        ...
        let stdout = child.stdout.take().ok_or_else(|| io::Error::other("task stdout pipe missing"))?;
        let stderr = child.stderr.take().ok_or_else(|| io::Error::other("task stderr pipe missing"))?;
        ...
        let (stdout, stdout_truncated) = stdout_reader.join().map_err(|_| io::Error::other("task stdout reader panicked"))??;
        let (stderr, stderr_truncated) = stderr_reader.join().map_err(|_| io::Error::other("task stderr reader panicked"))??;
...
impl PtySession {
    ...
    fn spawn_with_directory(program: &str, arguments: &[&str], rows: u16, columns: u16, directory: Option<&Path>) -> Result<Self, WorkflowError> {
        ...
        let child = pair.slave.spawn_command(command).map_err(|error| WorkflowError::Pty(format!("spawn failed: {error}")))?;
        drop(pair.slave);
        let mut reader = pair.master.try_clone_reader().map_err(|error| WorkflowError::Pty(error.to_string()))?;
        let writer = pair.master.take_writer().map_err(|error| WorkflowError::Pty(error.to_string()))?;
...
fn resolve_pty_program(program: &str) -> PathBuf {
    ...
    std::env::var_os("PATH")
        .and_then(|path| std::env::split_paths(&path).map(|directory| directory.join(program)).find(|candidate| is_executable_file(candidate)))
        .unwrap_or(direct)
}
...
impl FramedProcess {
    fn spawn(spec: &TaskSpec, trusted: bool, protocol: &str, limit: usize) -> Result<Self, WorkflowError> {
        if !trusted {
            return Err(WorkflowError::UntrustedTask);
        }
        let mut command = task_command(spec);
        ...
        let mut child = command.spawn()?;
        let input = child.stdin.take().ok_or_else(|| io::Error::other(format!("{protocol} stdin pipe missing")))?;
        let output = child.stdout.take().ok_or_else(|| io::Error::other(format!("{protocol} stdout pipe missing")))?;
        if let Some(stderr) = child.stderr.take() {
            thread::Builder::new().name(format!("wren-{}-stderr", protocol.to_ascii_lowercase())).spawn(move || {
                let _ = read_bounded(stderr, 1024 * 1024);
            })?;
        }
        Ok(Self { child, input: BufWriter::new(input), output: BufReader::new(output), max_frame_bytes: limit.max(1) })
    }

    fn write_body(&mut self, body: &[u8], error: impl FnOnce(&'static str) -> WorkflowError) -> Result<(), WorkflowError> {
        if body.len() > self.max_frame_bytes {
            return Err(error("outbound frame exceeds limit"));
        }
    ...
    fn read_body(&mut self, error: impl Fn(&'static str) -> WorkflowError) -> Result<Vec<u8>, WorkflowError> {
        let mut content_length = None;
        let mut header_bytes = 0_usize;
        loop {
            let mut line = String::new();
            if self.output.read_line(&mut line)? == 0 {
                return Err(error("peer closed its output"));
            }
            header_bytes = header_bytes.saturating_add(line.len());
            if header_bytes > 16 * 1024 {
                return Err(error("header exceeds limit"));
            }
            if line == "\r\n" || line == "\n" {
                break;
            }
        ...
        let length = content_length.ok_or_else(|| error("missing Content-Length"))?;
        if length > self.max_frame_bytes {
            return Err(error("inbound frame exceeds limit"));
        }
...
impl DapClient {
    ...
    pub fn send(&mut self, message: &DapMessage) -> Result<(), WorkflowError> {
        let body = serde_json::to_vec(message)?;
        self.process.write_body(&body, |message| WorkflowError::Dap(message.into()))
    }

    pub fn receive(&mut self) -> Result<DapMessage, WorkflowError> {
        let body = self.process.read_body(|message| WorkflowError::Dap(message.into()))?;
        Ok(serde_json::from_slice(&body)?)
    }
...
pub fn lower_lsp_text_edits(
    ...
    if base_revision != current_revision {
        return Err(WorkflowError::StaleRevision { expected: base_revision, actual: current_revision });
    }
    let line_starts = std::iter::once(0).chain(text.match_indices('\n').map(|(byte, _)| byte + 1)).collect::<Vec<_>>();
    let edits = edits
        .into_iter()
        .map(|edit| {
            let start = lsp_position_to_byte(text, &line_starts, edit.range.start)?;
            let end = lsp_position_to_byte(text, &line_starts, edit.range.end)?;
            if start > end {
                return Err(WorkflowError::InvalidLspPosition { line: edit.range.end.line, character: edit.range.end.character });
            }
            Ok(Edit::new(start..end, edit.new_text))
        })
        .collect::<Result<Vec<_>, WorkflowError>>()?;
    Transaction::new(base_revision, edits.clone()).map_err(|error| WorkflowError::Transaction(error.to_string().into()))?;
    Ok(RevisionedEdits { document_id, base_revision, edits })
}

fn lsp_position_to_byte(text: &str, line_starts: &[usize], position: LspPosition) -> Result<usize, WorkflowError> {
    let line = usize::try_from(position.line).map_err(|_| WorkflowError::InvalidLspPosition { line: position.line, character: position.character })?;
    let column = usize::try_from(position.character).map_err(|_| WorkflowError::InvalidLspPosition { line: position.line, character: position.character })?;
    utf16_position_to_byte(text, line_starts, line, column)
        .map_err(|_| WorkflowError::InvalidLspPosition { line: position.line, character: position.character })
}
...
impl LspClient {
    pub fn spawn(spec: &TaskSpec, trusted: bool, max_frame_bytes: usize) -> Result<Self, WorkflowError> {
        Ok(Self { process: FramedProcess::spawn(spec, trusted, "LSP", max_frame_bytes)?, next_request_id: 1 })
    }

    pub fn notify(&mut self, method: &str, parameters: serde_json::Value) -> Result<(), WorkflowError> {
    ...
    pub fn request(&mut self, method: &str, parameters: serde_json::Value) -> Result<serde_json::Value, WorkflowError> {
    ...
    fn answer_server_request(&mut self, request: &serde_json::Value) -> Result<(), WorkflowError> {
        ...
        let result = match request.get("method").and_then(serde_json::Value::as_str).unwrap_or_default() {
            "workspace/configuration" => request
                .pointer("/params/items")
                .and_then(serde_json::Value::as_array)
                .map_or_else(|| serde_json::json!([]), |items| serde_json::Value::Array(vec![serde_json::Value::Null; items.len()])),
    ...
    pub fn did_open(&mut self, uri: &str, language_id: &str, version: i64, text: &str) -> Result<(), WorkflowError> {
        self.notify(
    ...
    pub fn did_change_full(&mut self, uri: &str, version: i64, text: &str) -> Result<(), WorkflowError> {
        self.notify(
    ...
    fn write_message(&mut self, message: &serde_json::Value) -> Result<(), WorkflowError> {
        let body = serde_json::to_vec(message)?;
        self.process.write_body(&body, |message| WorkflowError::ProcessFailed(format!("LSP {message}").into()))
    }

    fn read_message(&mut self) -> Result<serde_json::Value, WorkflowError> {
        let body = self.process.read_body(|message| WorkflowError::ProcessFailed(format!("LSP {message}").into()))?;
        Ok(serde_json::from_slice(&body)?)
    }
}
...
pub fn formatter_edits(
    ...
    let edits = if before == formatted { Vec::new() } else { vec![Edit::new(0..before.len(), formatted)] };
    Ok(RevisionedEdits { document_id, base_revision, edits })
}
...
pub fn run_formatter_process(
    ...
    run_formatter_until_cancelled(spec, trusted, document_id, base_revision, current_revision, input, || false)
}
...
pub fn run_formatter_until_cancelled(
    ...
    let output = TaskSupervisor::new(trusted).run_captured(spec, Some(input.as_bytes()), is_cancelled)?;
    if output.cancelled {
        return Err(WorkflowError::Cancelled);
    }
    if output.status != Some(0) {
        return Err(WorkflowError::ProcessFailed(String::from_utf8_lossy(&output.stderr).trim().to_owned().into()));
    }
    if output.truncated {
        return Err(WorkflowError::ProcessFailed("formatter output exceeded its declared bound".into()));
    }
    let formatted = String::from_utf8(output.stdout).map_err(|error| WorkflowError::ProcessFailed(error.to_string().into()))?;
...
pub enum LspResourceEdit {
    ...
    Rename { from: PathBuf, to: PathBuf, expected_source_identity: FileIdentity, expected_target: Option<FileIdentity> },
...
pub fn lower_lsp_workspace_edit(
    ...
    let document_edits = documents
        .into_iter()
        .map(|document| {
            let transaction = Transaction::new(document.base_revision, document.edits).map_err(|error| WorkflowError::Transaction(error.to_string().into()))?;
            Ok(DocumentMutation {
    ...
    let resource_ops = resources
        ...
        .map(|resource| match resource {
            LspResourceEdit::Create { path, expected_absent } => {
                ResourceOp::Create { path: path.to_string_lossy().into_owned().into_boxed_str(), expected_absent }
            }
            LspResourceEdit::Rename { from, to, expected_source_identity, expected_target } => ResourceOp::Rename {
                from: from.to_string_lossy().into_owned().into_boxed_str(),
                to: to.to_string_lossy().into_owned().into_boxed_str(),
                expected_source_identity,
                expected_target: expected_target.map_or(ExpectedTarget::Absent, ExpectedTarget::Identity),
            },
            LspResourceEdit::Delete { path, expected_identity } => {
                ResourceOp::Delete { path: path.to_string_lossy().into_owned().into_boxed_str(), expected_identity }
            }
        })
        .collect();
...
impl ExcerptBuffer {
    pub fn commit(&self, group_id: SemanticGroupId, replacements: &BTreeMap<DocumentId, Vec<Edit>>) -> Result<WorkspaceTransaction, WorkflowError> {
        ...
        for excerpt in &self.excerpts {
            ...
            let entry = grouped.entry(excerpt.document_id).or_insert((excerpt.lease_epoch, excerpt.base_revision));
            if entry.0 != excerpt.lease_epoch || entry.1 != excerpt.base_revision {
                return Err(WorkflowError::StaleRevision { expected: entry.1, actual: excerpt.base_revision });
            }
        }
        ...
        for (document_id, (lease_epoch, base_revision)) in grouped {
            ...
            let transaction = Transaction::new(base_revision, edits).map_err(|error| WorkflowError::Transaction(error.to_string().into()))?;
        ...
        Ok(WorkspaceTransaction { document_edits, resource_ops: Vec::new() })
    }
}
...
pub fn structural_search(language: &str, text: &str, pattern: &str, max_matches: usize) -> Result<Vec<StructuralMatch>, WorkflowError> {
    ...
    let language = language.parse::<SupportLang>().map_err(|error| WorkflowError::Structural(error.to_string().into()))?;
    let pattern = Pattern::try_new(pattern, language).map_err(|error| WorkflowError::Structural(error.to_string().into()))?;
        ...
        .map(|matched| {
            let metavariables = matched
                ...
                .filter_map(|variable| match variable {
                    MetaVariable::Capture(name, _) => matched.get_env().get_match(&name).map(|node| (name.into_boxed_str(), node.range())),
                    MetaVariable::MultiCapture(name) => {
                        let nodes = matched.get_env().get_multiple_matches(&name);
                        nodes.first().zip(nodes.last()).map(|(first, last)| (name.into_boxed_str(), first.range().start..last.range().end))
                    }
                    MetaVariable::Dropped(_) | MetaVariable::Multiple => None,
                })
                .collect();
            StructuralMatch { range: matched.range(), metavariables }
        })
        .collect())
}
...
pub fn structural_rewrite(language: &str, text: &str, pattern: &str, replacement: &str, max_edits: usize) -> Result<Vec<Edit>, WorkflowError> {
    ...
    let mut edits = document
        ...
        .map(|matched| {
            ...
            let insert = String::from_utf8(edit.inserted_text).map_err(|error| WorkflowError::Structural(error.to_string().into()))?;
            Ok(Edit::new(edit.position..edit.position.saturating_add(edit.deleted_length), insert))
        })
        .collect::<Result<Vec<_>, WorkflowError>>()?;
...
impl RevisionedEdits {
    pub fn accept(&self, selected: &[usize], current_revision: DocumentRevision, intervening: &[Transaction]) -> Result<Transaction, WorkflowError> {
        ...
        for index in selected {
            let mut edit = self.edits.get(*index).cloned().ok_or(WorkflowError::Unmappable)?;
            for transaction in intervening {
                edit.range.start = transaction.map_offset(edit.range.start, Bias::Left).map_err(|_| WorkflowError::Unmappable)?;
                edit.range.end = transaction.map_offset(edit.range.end, Bias::Right).map_err(|_| WorkflowError::Unmappable)?;
            }
            edits.push(edit);
        }
        edits.sort_by_key(|edit| edit.range.start);
        Transaction::new(current_revision, edits).map_err(|error| WorkflowError::Transaction(error.to_string().into()))
    }
}
...
mod tests {
    ...
    fn task(program: &str, arguments: Vec<Box<str>>, max_output_bytes: usize) -> TaskSpec {
        TaskSpec::persisted(resolve_pty_program(program), arguments, BTreeMap::new(), max_output_bytes)
    }

    fn path_task(program: &str, arguments: Vec<Box<str>>, max_output_bytes: usize) -> TaskSpec {
        TaskSpec::persisted(resolve_pty_program(program), arguments, inherited_path_environment(), max_output_bytes)
    }
    ...
    fn task_visibility_is_explicit_and_output_is_bounded() {
        ...
        assert!(TaskSupervisor::new(false).run(&task("true", Vec::new(), 1)).is_err());
    }
    ...
    fn task_cancellation_terminates_the_whole_process_group() {
        ...
        let output = TaskSupervisor::new(true)
            .run_until_cancelled(&path_task("sh", vec!["-c".into(), "sleep 5 & wait".into()], 16), || cancelled.try_recv().is_ok())
            .expect("cancel task");
    ...
    fn task_cancellation_escalates_when_sigterm_is_ignored() {
        ...
        let started = Instant::now();
        let output = TaskSupervisor::new(true)
            .run_until_cancelled(&path_task("sh", vec!["-c".into(), "trap '' TERM; while :; do sleep 1; done".into()], 16), || cancelled.try_recv().is_ok())
    ...
    fn live_pty_can_start_in_an_explicit_workspace() {
        ...
        let mut session = PtySession::spawn_in("sh", &["-c", "printf '%s' \"$PWD\""], 4, 80, directory.path()).expect("spawn PTY in workspace");
        let deadline = Instant::now() + Duration::from_secs(2);
        while session.exit_code().is_none() && Instant::now() < deadline {
            session.poll().expect("poll");
            thread::yield_now();
    ...
    fn native_dap_client_spawns_adapter_and_exchanges_framed_messages() {
        ...
        let mut client = DapClient::spawn(&task("sh", vec!["-c".into(), script.into()], 1024), true, 1024).expect("spawn adapter");
        let request = DapMessage { seq: 4, body: json!({"type":"request","command":"continue"}) };
    ...
    fn native_lsp_client_spawns_and_correlates_json_rpc_responses() {
        ...
        let mut client = LspClient::spawn(&task("sh", vec!["-c".into(), script.into()], 1024), true, 1024).expect("spawn language server");
        let result = client.request("initialize", serde_json::json!({"capabilities": {}})).expect("LSP request");
    ...
    fn formatter_lsp_excerpt_and_ai_edits_stay_revisioned() {
        assert!(matches!(
            formatter_edits(DocumentId::new(1), DocumentRevision::new(1), DocumentRevision::new(2), "a", "b"),
    ...
    fn formatter_process_is_bounded_and_revision_validated() {
        let formatted = run_formatter_process(
            &task("tr", vec!["a-z".into(), "A-Z".into()], 1024),
            true,
            DocumentId::new(1),
    ...
    fn git_and_structural_results_are_native_and_bounded() {
        ...
        let edits = structural_rewrite("rust", "fn one() { Some(1); }", "Some($A)", "Ok($A)", 8).expect("ast-grep rewrite");
        let rewritten =
            Transaction::new(DocumentRevision::new(1), edits).expect("rewrite transaction").apply_to_string("fn one() { Some(1); }").expect("apply rewrite");
