...
impl ConfigBundle {
    ...
    pub fn new(
        ...
        Self { config_toml, query_sources, theme_sources, language_bundles, content_hash: *hasher.finalize().as_bytes() }
    }
}
...
pub enum ConfigError {
    ...
    MissingArgument { command: Box<str>, argument: Box<str> },
    #[error("command {command} has unknown argument {argument}")]
    UnknownArgument { command: Box<str>, argument: Box<str> },
    #[error("command {command} argument {argument} has the wrong type")]
    ArgumentType { command: Box<str>, argument: Box<str> },
...
impl CommandRegistry {
    ...
    pub fn new(schemas: impl IntoIterator<Item = CommandSchema>) -> Self {
        Self { schemas: schemas.into_iter().map(|schema| (schema.name.clone(), schema)).collect() }
    }
...
pub fn sanitized_environment(inherited: impl IntoIterator<Item = (String, String)>) -> BTreeMap<String, String> {
    ...
    inherited.into_iter().filter(|(name, _)| exact.contains(name.as_str()) || name.starts_with("LC_")).collect()
}

fn has_executable_contributions(config: &Config) -> bool {
    !config.tasks.is_empty() || !config.extensions.is_empty() || config.environment.direnv.unwrap_or(false) || config.environment.nix.unwrap_or(false)
}
...
fn command_value(value: &toml::Value, expected: &CommandArgumentType) -> Option<CommandValue> {
    match expected {
        ...
        CommandArgumentType::Number => value.as_float().or_else(|| value.as_integer().map(|value| value as f64)).map(CommandValue::Number),
        CommandArgumentType::String => value.as_str().map(|value| CommandValue::String(value.into())),
        CommandArgumentType::StringList => value
            .as_array()
            .and_then(|values| values.iter().map(|value| value.as_str().map(Box::<str>::from)).collect::<Option<Vec<_>>>().map(CommandValue::StringList)),
        CommandArgumentType::Enumeration(allowed) => {
            value.as_str().and_then(|value| allowed.iter().any(|candidate| candidate.as_ref() == value).then(|| CommandValue::String(value.into())))
        }
    }
}
        ...
        CommandRegistry::new([CommandSchema {
            ...
            arguments: vec![
                CommandArgumentSchema {
                    name: "root".into(),
                    argument_type: CommandArgumentType::Enumeration(vec!["workspace".into(), "cwd".into()]),
                    required: true,
                    description: "root".into(),
                },
                CommandArgumentSchema { name: "hidden".into(), argument_type: CommandArgumentType::Boolean, required: false, description: "hidden".into() },
            ],
        }])
    }
...
hidden = false
"#;
        let config = parse_and_validate(source, &registry(), WorkspaceTrust::Untrusted).expect("valid config");
        assert_eq!(config.keys["normal"]["space f"].command, "picker.files");
        assert!(parse_and_validate(&source.replace("workspace", "invalid"), &registry(), WorkspaceTrust::Untrusted).is_err());
        assert!(parse_and_validate(&source.replace("language == 'rust'", "read_file('secret')"), &registry(), WorkspaceTrust::Untrusted).is_err());
    }
...
"#;
        ...
        parse_and_validate(source, &registry(), WorkspaceTrust::Trusted { executable_hash: hash }).expect("trusted");
        assert!(!WorkspaceTrust::Trusted { executable_hash: hash }.allows(*blake3::hash(b"changed").as_bytes()));
    }
