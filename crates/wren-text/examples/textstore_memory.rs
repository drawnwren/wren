...
fn profile<T: TextStore>(path: &str) -> anyhow::Result<()> {
    ...
    println!("steady_bytes={} peak_bytes={} total_bytes={}", stats.curr_bytes, stats.max_bytes, stats.total_bytes);
    Ok(())
}
...
fn main() -> anyhow::Result<()> {
    ...
    let path = arguments.get(2).ok_or_else(|| anyhow::anyhow!("usage: textstore_memory BACKEND CORPUS"))?;
