...
use wren_types::{
    ClientId, ClientMutation, ClientSequence, DocumentId, DocumentMutation, DocumentRevision, Edit, LeaseEpoch, MutationId, MutationResult, SaveRequest,
    SemanticGroupId, SemanticGroupKind, SessionId, SessionSequence, Transaction, WorkspaceGeneration,
};
...
fn test_ssh_spec(temporary: &Path, workspace: &Path, state: &Path) -> OpenSshSpec {
    ...
    fs::set_permissions(&fake_ssh, fs::Permissions::from_mode(0o700)).expect("make fake ssh executable");
    OpenSshSpec {
        executable: fake_ssh,
        host: "test-host".into(),
        user: None,
...
fn assert_remote_open_semantics(client: &mut RemoteWorkspaceClient) -> RemoteOpened {
    let manifest = client.manifest(WorkspaceGeneration::new(1)).expect("remote manifest");
    assert!(manifest.entries.contains_key("main.rs"));

    let opened = client.open(DocumentId::new(9), ClientId::new(7), "main.rs", None).expect("remote open");
    assert!(!opened.cached_hash_valid);
    assert_eq!(client.blob(opened.content_hash).expect("bulk blob"), b"fn main() {}\n");
    let cached = client.open(DocumentId::new(9), ClientId::new(7), "main.rs", Some(opened.content_hash)).expect("cached reopen");
    assert!(cached.cached_hash_valid);
    assert!(matches!(client.open(DocumentId::new(9), ClientId::new(7), "other.rs", None), Err(wren_remote::RemoteError::Peer(_))));
    opened
}
...
fn test_mutation() -> ClientMutation {
    ClientMutation {
        ...
        documents: vec![DocumentMutation {
            ...
            undo_parent: None,
            transactions: vec![Transaction::new(DocumentRevision::new(0), vec![Edit::new(13..13, "// remote\n")]).expect("transaction")],
        }],
    }
}

fn submit_and_save(client: &mut RemoteWorkspaceClient, opened: &RemoteOpened, workspace: &Path) -> SessionSequence {
    let MutationResult::Durable { session_sequence, .. } = client.submit(&test_mutation()).expect("durable mutation") else {
    ...
    assert_eq!(fs::read_to_string(workspace.join("main.rs")).expect("saved source"), "fn main() {}\n// remote\n");
    session_sequence
}

fn assert_search_and_resume(client: &mut RemoteWorkspaceClient, opened: &RemoteOpened, session_sequence: SessionSequence) {
    ...
    let resumed = client
        .reconnect(&wren_types::Resume {
            ...
            document_frontiers: vec![wren_types::DocumentFrontier { document_id: DocumentId::new(9), revision: DocumentRevision::new(1) }],
            outstanding_mutation_ids: Vec::new(),
        })
        .expect("application-level resume over replacement lanes");
    assert!(matches!(resumed, wren_types::ResumeResult::Replay { .. } | wren_types::ResumeResult::SnapshotRequired { .. }));
    client.heartbeat(SessionSequence::new(42).get()).expect("heartbeat after reconnect");
}
