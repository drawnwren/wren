...
use wren_session::{AuthorityError, LocalDocument, SaveError, SessionAuthority};
...
impl SessionServer {
    ...
    pub fn authority(&self) -> MutexGuard<'_, SessionAuthority> {
        self.authority.lock()
    }
    ...
    where
        S: Read + Write,
    {
        let hello_envelope = read_envelope(stream, DEFAULT_MAX_FRAME_BYTES)?.ok_or(ServerError::ExpectedHello)?;
        ...
        let negotiated_limit = DEFAULT_MAX_FRAME_BYTES.min(client_limit.max(1));
        let capabilities =
    ...
    fn serve_request(&self, stream: &mut (impl Read + Write), request: Envelope, limit: usize) -> Result<(), ServerError> {
        ...
        match request.payload.ok_or(ProtocolError::MissingField("Envelope.payload"))? {
            envelope::Payload::ClientMutation(mutation) => self.handle_mutation(stream, request_id, mutation, limit),
            ...
            envelope::Payload::SaveRequest(request) => {
                let saved = self.save_remote_document(&request)?;
                self.write_payload(stream, request_id, envelope::Payload::Saved(saved), limit)
            }
            payload => Err(ServerError::UnexpectedPayload(format!("{payload:?}").into())),
        }
    }
        ...
        match submission {
            Ok(durable) => {
                ...
                for event in self.authority().events_after(previous_sequence) {
                    self.write_payload(stream, 0, envelope::Payload::SessionEvent(event), limit)?;
                }
                Ok(())
            }
            Err(result) => self.send_result(stream, request_id, &result, limit),
        }
    }

    fn handle_resume(&self, stream: &mut impl Write, request_id: u64, resume: wren_proto::Resume, limit: usize) -> Result<(), ServerError> {
        let result = self.authority().resume(&resume);
        self.write_payload(stream, request_id, envelope::Payload::ResumeResult(result), limit)
    }
    ...
    fn send_result(&self, stream: &mut impl Write, request_id: u64, result: &wren_types::MutationResult, limit: usize) -> Result<(), ServerError> {
        self.write_payload(stream, request_id, envelope::Payload::MutationResult(result.clone()), limit)
    }
    ...
    fn send_remote_reply(&self, stream: &mut impl Write, request_id: u64, reply: Result<RemoteReply, ServerError>, limit: usize) -> Result<(), ServerError> {
        ...
        self.write_payload(stream, request_id, envelope::Payload::RemoteReply(wren_proto::RemoteReply { body: serde_json::to_vec(&reply)? }), limit)
    }

    fn handle_remote_call(&self, call: RemoteCall) -> Result<RemoteReply, ServerError> {
        let workspace = self.remote_workspace(call.lane())?;
        match call {
            RemoteCall::Heartbeat { nonce } => Ok(RemoteReply::Heartbeat { nonce }),
    ...
    fn handle_remote_open(
        ...
        let file_identity = document.stamp().map(|stamp| stamp.identity).unwrap_or(wren_types::FileIdentity { device: 0, file: 0, generation: 0 });
        let content_hash = {
    ...
    fn save_remote_document(&self, request: &wren_types::SaveRequest) -> Result<wren_types::Saved, ServerError> {
        let workspace = self.remote_workspace(TransportLane::Control)?;
        let (text, revision) = {
            let authority = self.authority();
        ...
        let expected_identity = document.stamp().map(|stamp| stamp.identity).unwrap_or(wren_types::FileIdentity { device: 0, file: 0, generation: 0 });
        ...
        Ok(wren_types::Saved { document_id: request.document_id, persisted_frontier: revision, new_file_identity: report.stamp.identity, new_content_hash })
    }

    fn remote_workspace(&self, lane: TransportLane) -> Result<&Arc<Mutex<RemoteWorkspace>>, ServerError> {
        let workspace = self.remote_workspace.as_ref().ok_or_else(|| ServerError::UnexpectedPayload("remote request on a local-only session".into()))?;
        if workspace.lock().lane == lane {
            Ok(workspace)
        } else {
            Err(ServerError::UnexpectedPayload("remote request arrived on the wrong transport lane".into()))
        }
    }
...
mod tests {
    ...
    use wren_proto::{Hello, MutationResult as WireResult, OpenDocument};
    ...
    fn assert_received_then_durable(client: &mut UnixStream) {
        write_envelope(client, &Envelope::new(11, envelope::Payload::ClientMutation(mutation())), DEFAULT_MAX_FRAME_BYTES).expect("mutation");
        ...
        assert!(matches!(first, MutationResult::Received { .. }));
        assert!(matches!(second, MutationResult::Durable { .. }));
    ...
    fn crash_after_received_leaves_no_commit_and_retry_is_durable() {
        ...
        write_envelope(&mut client, &Envelope::new(2, envelope::Payload::ClientMutation(mutation())), DEFAULT_MAX_FRAME_BYTES).expect("mutation");
        ...
        assert!(matches!(received, MutationResult::Received { .. }));
        drop(client);
        ...
        assert!(matches!(recovered.submit(mutation()).expect("retry"), Ok(MutationResult::Durable { .. })));
