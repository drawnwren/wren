...
/// deterministic visual seeds. Callers choose the byte framing, so the same
/// primitive serves paths, virtual documents, and numeric geometry keys.
#[must_use]
pub fn stable_hash(bytes: impl IntoIterator<Item = u8>) -> u64 {
    bytes.into_iter().fold(0xcbf2_9ce4_8422_2325, |hash, byte| (hash ^ u64::from(byte)).wrapping_mul(0x0000_0100_0000_01b3))
}
...
impl Transaction {
    ...
    fn map_offset_validated(&self, byte: usize, bias: Bias) -> Result<usize, TransactionError> {
        ...
        while index < self.edits.len() {
            ...
            let start = i128::try_from(edit.range.start).map_err(|_| TransactionError::OffsetOverflow)?;
            let end = i128::try_from(edit.range.end).map_err(|_| TransactionError::OffsetOverflow)?;
            let direct_insert_len = i128::try_from(edit.insert.len()).map_err(|_| TransactionError::OffsetOverflow)?;
            ...
            if start < end {
                while index + consumed < self.edits.len() {
                    ...
                    if candidate.range.start != edit.range.end || candidate.range.end != edit.range.end {
                        break;
                    }
                    insert_len += i128::try_from(candidate.insert.len()).map_err(|_| TransactionError::OffsetOverflow)?;
    ...
    pub fn then(&self, next: &Self) -> Result<Self, TransactionError> {
        ...
        if next.base_revision != expected {
            return Err(TransactionError::RevisionMismatch { expected: expected.get(), actual: next.base_revision.get() });
        }

        let self_extent = self.edits.iter().map(|edit| edit.range.end).max().unwrap_or(0);
        let next_extent = next.edits.iter().map(|edit| edit.range.end).max().unwrap_or(0);
        let deleted = self.edits.iter().try_fold(0_usize, |sum, edit| sum.checked_add(edit.range.len())).ok_or(TransactionError::OffsetOverflow)?;
        let inserted = self.edits.iter().try_fold(0_usize, |sum, edit| sum.checked_add(edit.insert.len())).ok_or(TransactionError::OffsetOverflow)?;
        let mut base_extent = self_extent;
        let post_extent = base_extent.checked_sub(deleted).and_then(|value| value.checked_add(inserted)).ok_or(TransactionError::OffsetOverflow)?;
        if post_extent < next_extent {
            base_extent = base_extent.checked_add(next_extent - post_extent).ok_or(TransactionError::OffsetOverflow)?;
        }
        ...
        composed.composition = Some(Box::new(Composition { first: self.clone(), second: next.clone() }));
        Ok(composed)
    }
    ...
    pub fn invert(&self, deleted_text: &[Box<str>]) -> Result<Self, TransactionError> {
        if deleted_text.len() != self.edits.len() {
            return Err(TransactionError::DeletedTextCount { expected: self.edits.len(), actual: deleted_text.len() });
        }
    ...
    fn symbolic_pieces(&self, base_extent: usize) -> Result<Vec<Piece>, TransactionError> {
        ...
        for edit in &self.edits {
            if edit.range.end > base_extent {
                return Err(TransactionError::OutOfBounds { offset: edit.range.end, len: base_extent });
            }
...
impl SelectionSet {
    pub fn validate(&self) -> Result<(), SelectionError> {
        ...
        if self.primary >= self.ranges.len() {
            return Err(SelectionError::PrimaryOutOfBounds { primary: self.primary, len: self.ranges.len() });
        }
        Ok(())
    }
...
pub enum FreshnessKey {
    Document { document_id: DocumentId, document_revision: DocumentRevision, provider_generation: ProviderGeneration },
    Workspace { kind: WorkspaceGenerationKind, workspace_generation: WorkspaceGeneration, provider_generation: ProviderGeneration },
}
...
pub enum RemoteOpenState {
    ...
    CachedAwaitingHead { cached_revision: DocumentRevision },
    Progressive { authoritative_revision: DocumentRevision, received_bytes: u64, total_bytes: u64 },
...
impl RemoteOpenState {
    ...
    pub const fn frame_authority(&self) -> RemoteFrameAuthority {
        match self {
            Self::CachedHeadValidated { .. } | Self::Materialized { .. } => RemoteFrameAuthority::Correct,
            Self::CachedAwaitingHead { .. } | Self::Progressive { .. } => RemoteFrameAuthority::Speculative,
        }
    }
...
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum StateDelta {
    Register { name: char, text: Box<str>, linewise: bool },
    SearchPattern(Box<str>),
    SearchDirection { backward: bool },
    CommandHistory(Box<str>),
    GlobalMark { name: char, document_id: DocumentId, anchor: Anchor },
    UndoBranchHead { document_id: DocumentId, semantic_group_id: Option<SemanticGroupId> },
    RepeatData(Vec<u8>),
    MacroRecording { name: char, raw_keys: Vec<u8>, lowered_ir: Vec<u8> },
    JumpList { entries: Vec<DurableJumpEntry>, current: Option<usize> },
}
...
impl DocumentMutation {
    ...
    pub fn accepted_revision(&self) -> Result<DocumentRevision, MutationValidationError> {
        self.validate()?;
        let count = u64::try_from(self.transactions.len()).map_err(|_| MutationValidationError::RevisionOverflow)?;
        self.base_revision.get().checked_add(count).map(DocumentRevision::new).ok_or(MutationValidationError::RevisionOverflow)
    }
}
...
pub enum MutationValidationError {
    ...
    #[error("document {document_id:?} transaction chain expected revision {expected}, got {actual}")]
    RevisionChain { document_id: DocumentId, expected: u64, actual: u64 },
...
pub enum MutationResult {
    Received { mutation_id: MutationId },
    Durable { mutation_id: MutationId, client_sequence: ClientSequence, session_sequence: SessionSequence, documents: Vec<AcceptedDocument> },
    RebaseRequired { mutation_id: MutationId, document_id: DocumentId, authoritative_revision: DocumentRevision, delta_since_base: Vec<Transaction> },
    LeaseLost { document_id: DocumentId, current_lease_epoch: LeaseEpoch },
    Conflict { document_id: DocumentId, reason: Box<str> },
}
...
pub enum SessionEventPayload {
    DocumentDelta { document_id: DocumentId, accepted_revision: DocumentRevision, transactions: Vec<Transaction> },
    StateDelta(StateDelta),
    LeaseChange(LeaseGrant),
    ExternalChange { document_id: DocumentId, content_hash: [u8; 32] },
}
...
pub enum ResourceOp {
    ...
    Rename { from: Box<str>, to: Box<str>, expected_source_identity: FileIdentity, expected_target: ExpectedTarget },
...
fn apply_symbolic_edits(pieces: &[Piece], edits: &[Edit], base_extent: usize) -> Result<Vec<Piece>, TransactionError> {
    ...
    for edit in edits {
        if edit.range.end > total_len {
            return Err(TransactionError::OutOfBounds { offset: edit.range.end, len: total_len });
        }
...
fn slice_pieces(pieces: &[Piece], wanted: Range<usize>) -> Result<Vec<Piece>, TransactionError> {
    ...
    for piece in pieces {
        let piece_end = offset.checked_add(piece.len()).ok_or(TransactionError::OffsetOverflow)?;
        ...
        if start < end {
            ...
            match piece {
                Piece::Base(range) => output.push(Piece::Base((range.start + local_start)..(range.start + local_end))),
                Piece::Inserted { text, anchor } => {
                    let slice = text.get(local_start..local_end).ok_or(TransactionError::NotCharBoundary { offset: local_start })?;
                    output.push(Piece::Inserted { text: Box::from(slice), anchor: *anchor });
                }
            }
        }
        offset = piece_end;
...
fn coalesce_pieces(pieces: Vec<Piece>) -> Vec<Piece> {
    ...
    for piece in pieces {
        match (output.last_mut(), piece) {
            ...
            (Some(Piece::Inserted { text: previous, anchor: previous_anchor }), Piece::Inserted { text: next, anchor: next_anchor })
    ...
    for piece in pieces {
        match piece {
            Piece::Inserted { text, anchor } => {
                ...
                if *anchor > base_cursor {
                    edits.push(Edit::new(base_cursor..*anchor, std::mem::take(&mut inserted)));
                    base_cursor = *anchor;
                }
            ...
            Piece::Base(range) => {
                ...
                if range.start > base_cursor || !inserted.is_empty() {
                    edits.push(Edit::new(base_cursor..range.start, std::mem::take(&mut inserted)));
                }
...
fn anchor_at_position(pieces: &[Piece], position: usize, base_extent: usize) -> Result<usize, TransactionError> {
    let mut offset = 0_usize;
    for piece in pieces {
        let end = offset.checked_add(piece.len()).ok_or(TransactionError::OffsetOverflow)?;
        if position < end {
            return match piece {
    ...
    if position == offset { Ok(base_extent) } else { Err(TransactionError::OutOfBounds { offset: position, len: offset }) }
}
...
mod tests {
    ...
    fn anchor_bias_controls_insert_boundary() {
        let transaction = Transaction::new(DocumentRevision::new(4), vec![Edit::new(2..2, "xyz")]).expect("valid transaction");
    ...
    fn mapping_equivalence_ignores_only_the_base_revision() {
        let first = Transaction::new(DocumentRevision::new(4), vec![Edit::new(2..2, "xyz")]).expect("first transaction");
        let same_mapping = Transaction::new(DocumentRevision::new(9), vec![Edit::new(2..2, "xyz")]).expect("equivalent transaction");
        let different = Transaction::new(DocumentRevision::new(9), vec![Edit::new(2..2, "xy")]).expect("different transaction");
    ...
    fn transaction_deserialization_preserves_structural_invariants() {
        ...
        let error = serde_json::from_str::<Transaction>(invalid).expect_err("overlapping edits must not deserialize");
        assert!(error.to_string().contains("overlaps"));

        let original = Transaction::new(DocumentRevision::new(4), vec![Edit::new(2..5, "replacement")]).expect("valid transaction");
    ...
    fn selection_mapping_preserves_range_edges() {
        let transaction = Transaction::new(DocumentRevision::new(0), vec![Edit::new(2..4, "long")]).expect("valid transaction");
    ...
    fn semantic_mutation_requires_a_contiguous_revision_chain() {
        ...
        let mutation = ClientMutation {
            ...
            documents: vec![DocumentMutation {
                ...
                undo_parent: None,
                transactions: vec![
                    Transaction::new(DocumentRevision::new(8), vec![Edit::new(0..0, "a")]).expect("first transaction"),
                    Transaction::new(DocumentRevision::new(10), vec![Edit::new(1..1, "b")]).expect("second transaction"),
                ],
            }],
        };
        assert_eq!(mutation.validate(), Err(MutationValidationError::RevisionChain { document_id, expected: 9, actual: 10 }));
    }
    ...
    fn document_classification_accounts_for_size_lines_sampling_and_generation() {
        let base = DocumentProfile { byte_length: 1_024, longest_line_estimate: 80, parse_bytes_per_millisecond: Some(1_000_000), generated_file: false };
        assert_eq!(DocumentClass::classify(base), DocumentClass::Normal);
        assert_eq!(DocumentClass::classify(DocumentProfile { generated_file: true, ..base }), DocumentClass::Large);
        assert_eq!(
            DocumentClass::classify(DocumentProfile { longest_line_estimate: DocumentClass::PATHOLOGICAL_LINE_BYTES, ..base }),
